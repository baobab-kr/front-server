...
export default function PersonPage(): JSX.Element {
  ...
  return (
    <Container>
      <TagPlace>
        <div style={{ margin: "8px 0px" }}>태그 목록</div>
        <hr />
        <ul>
      ...
      <div>
        <UserInfo>
          <div style={{ width: "150px", height: "150px", borderRadius: "70%", overflow: "hidden", justifySelf: "start" }}>
            <img src={DefaultAvator} style={{ background: "yellow", width: "100%", height: "100%", objectFit: "cover" }} alt="avator"></img>
          </div>
          <div style={{ display: "flex", gap: "10px", flexDirection: "column" }}>
            <div style={{ color: "black" }}>
              <h1 style={{ fontSize: "50px" }}>Baobab</h1>
            </div>
            <hr />
        ...
        <Content>
          {blog.map((blogInfo, index) => {
            return <PersonCard key={index} borderID={index} imgSrc={image} board={blogInfo} />;
          })}
        </Content>
      </div>
      <div>
